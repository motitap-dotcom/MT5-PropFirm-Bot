#!/bin/bash
# Trigger: v132 - ONLY browser auth, no restart
cd /root/MT5-PropFirm-Bot
source .env
echo "=== Browser Auth Only v132 ==="
...
async def main():
        ...
        async def on_resp(resp):
            ...
            try:
                if "auth" in resp.url.lower() and resp.status == 200:
                    d = await resp.json()
                    if isinstance(d, dict) and "accessToken" in d: captured = d
            except: pass
    ...
    if captured:
        ...
        print(f"TOKEN SAVED - expires {captured.get('expirationTime','?')}")
    else: print("FAILED")
asyncio.run(main())
PYEOF
